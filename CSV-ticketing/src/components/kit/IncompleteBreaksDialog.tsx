...
export const IncompleteBreaksDialog = () => {
  ...
  // Function to determine color based on break type and duration
  const getTimeConsumedColor = (breakType: string, seconds: number): string => {
    if (breakType.toLowerCase().includes("lunch")) {
      return seconds < 3600
      ...
      return seconds < 900
        ? "bg-green-100 text-green-800"
        : "bg-red-100 text-red-800"; // 900 seconds = 15 minutes
    }
  };
  ...
  const fetchIncompleteBreaks = async (isBackgroundRefresh = false) => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data.data)) {
        const newBreaks = response.data.data;
        ...
        throw new Error("No breaks found. All breaks have been ended!");
      }
    } catch (err) {
      ...
      if (err instanceof Error) {
        ...
        setError("Failed to fetch breaks. Please try again.");
      }
    } finally {
      if (!isBackgroundRefresh) {
        setLoading(false);
            ...
            <div className="overflow-auto max-h-[60vh] relative">
              {error && (
                <div className="text-center py-8 text-gray-500">
                  ...
                  No breaks found. All breaks have been ended!
                </div>
              )}
                  ...
                  <table className="min-w-full divide-y divide-gray-200 border">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {breakData.breaks.map((breakItem, index) => {
                        ...
                        const colorClass = getTimeConsumedColor(
                          breakItem.type,
                          seconds
                        );
