...
import CreateNODDialog from "@/components/createNodDialog";
...
interface PnodTableProps {
  onView?: (id: string) => void;
  onCreateNod?: (id: string) => void;
}

const PnodTable: React.FC<PnodTableProps> = ({ onCreateNod }) => {
  ...
  const [showCreateNODDialog, setShowCreateNODDialog] =
  ...
  const handleCreateNod = (item: NteItem) => {
    setSelectedNte(item);
    setShowCreateNODDialog(true);
  };

  const handleCreateNODSuccess = () => {
    if (onCreateNod) {
      onCreateNod(selectedNte!._id);
    }
    setShowCreateNODDialog(false);
    fetchData();
  };
  ...
  const getStatusColor = (status: NteItem["status"]): string => {
    const statusColors = {
      PNOD: "bg-orange-100 text-orange-800",
    };
  ...
  return (
    <div className="w-full overflow-x-auto px-6">
      ...
      {selectedNte && (
        <CreateNODDialog
          open={showCreateNODDialog}
          onOpenChange={setShowCreateNODDialog}
          nteId={selectedNte._id}
          nteData={selectedNte}
          onCreateSuccess={handleCreateNODSuccess}
        />
      )}
    </div>
  );
